...
Audio generation uses `Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice` by default. WAV files preserve 32-bit float audio, and MP3 export uses ffmpeg/LAME's highest VBR quality setting.
...
Qwen TTS runs locally through PyTorch. Use `tts_mode: "cuda"` on NVIDIA machines, `tts_mode: "apple"` on Apple Silicon with PyTorch MPS, or leave it as `"auto"` to prefer CUDA and then Apple MPS. The first run may download Qwen model weights into the Hugging Face cache.
